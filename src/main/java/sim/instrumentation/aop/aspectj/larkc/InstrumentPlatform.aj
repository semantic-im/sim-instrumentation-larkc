 ...
 *
 */
public aspect InstrumentPlatform extends AbstractMethodInterceptor {

	public pointcut methodExecution(): within(eu.larkc.core.Larkc) && execution(public static void main(..));
	
	protected void beforeInvoke(JoinPoint jp) {
		ExecutionFlowContext.createNewContext().
			put("platformid", UUID.randomUUID().toString()).
			put("tag", "platform").
			put("name", "PlatformStart");
	} 
